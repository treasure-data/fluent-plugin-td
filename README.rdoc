...
* +auto_create_table+
  ...
  *NOTE*::
  depending on the access control permissions associated
  to the API key, a database can or cannot be created if not already available.
...
* +tmpdir+
  specifies the temporary folder to use for storing the temporarily converted
